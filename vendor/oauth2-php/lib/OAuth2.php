...
class OAuth2 {
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		switch ($input["grant_type"]) {
			...
			case filter_var($input["grant_type"], FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => self::GRANT_TYPE_REGEXP), "flags" => FILTER_REQUIRE_SCALAR)):
				if (!($this->storage instanceof IOAuth2GrantExtension)) {
					throw new OAuth2ServerException(self::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				$stored = $this->storage->checkGrantExtension($input["grant_type"], $inputData, $authHeaders);
